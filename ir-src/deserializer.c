...
static void load_expr_data(IrExpr *expr, u8 *data, u32 *end,
  ...
  *end += sizeof(u8);
  ...
  case IrExprKindFuncCall: {
    ...
    load_expr_data(expr->as.func_call.func, data, end, path_offsets, arena, persistent_arena);
    load_block_data(&expr->as.func_call.args, data, end, path_offsets, arena, persistent_arena);
  } break;
  ...
  case IrExprKindVarDef: {
    ...
    load_expr_data(expr->as.var_def.expr, data, end, path_offsets, arena, persistent_arena);
  } break;

  case IrExprKindIf: {
    expr->as._if.cond = arena_alloc(arena, sizeof(IrExpr));

    load_expr_data(expr->as._if.cond, data, end, path_offsets, arena, persistent_arena);
    ...
    *end += sizeof(u32);
    ...
    for (u32 i = 0; i < expr->as._if.elifs.len; ++i) {
      ...
      load_expr_data(elif.cond, data, end, path_offsets, arena, persistent_arena);
      load_block_data(&elif.body, data, end, path_offsets, arena, persistent_arena);
    ...
    *end += sizeof(u8);
    ...
    if (expr->as._if.has_else)
      load_block_data(&expr->as._if.else_body, data, end, path_offsets, arena, persistent_arena);
  } break;

  case IrExprKindWhile: {
    expr->as._while.cond = arena_alloc(arena, sizeof(IrExpr));

    load_expr_data(expr->as._while.cond, data, end, path_offsets, arena, persistent_arena);
    load_block_data(&expr->as._while.body, data, end, path_offsets, arena, persistent_arena);
  } break;
  ...
  case IrExprKindSet: {
    ...
    load_expr_data(expr->as.set.src, data, end, path_offsets, arena, persistent_arena);
  } break;
  ...
  case IrExprKindGetAt: {
    ...
    load_expr_data(expr->as.get_at.src, data, end, path_offsets, arena, persistent_arena);
    load_expr_data(expr->as.get_at.key, data, end, path_offsets, arena, persistent_arena);
  } break;
  ...
  case IrExprKindSetAt: {
    ...
    load_expr_data(expr->as.set_at.key, data, end, path_offsets, arena, persistent_arena);
    load_expr_data(expr->as.set_at.value, data, end, path_offsets, arena, persistent_arena);
  } break;
  ...
  case IrExprKindRet: {
    ...
    *end += sizeof(u8);
    ...
    if (expr->as.ret.has_expr) {
      ...
      load_expr_data(expr->as.ret.expr, data, end, path_offsets, arena, persistent_arena);
    }
  } break;
  ...
  case IrExprKindLambda: {
    ...
    *end += sizeof(u32);
    ...
    load_block_data(&expr->as.lambda.body, data, end, path_offsets,
  ...
  case IrExprKindDict: {
    ...
    *end += sizeof(u32);
    ...
    for (u32 i = 0; i < expr->as.dict.len; ++i) {
      ...
      load_expr_data(expr->as.dict.items[i].key, data, end, path_offsets, arena, persistent_arena);
      load_expr_data(expr->as.dict.items[i].expr, data, end, path_offsets, arena, persistent_arena);
    }
  } break;
    ...
    *end += sizeof(u32);
    ...
    for (u32 i = 0; i < expr->as.match.cases.len; ++i) {
      ...
      load_expr_data(expr->as.match.cases.items[i].pattern, data,
                     end, path_offsets, arena, persistent_arena);
...
static void load_block_data(IrBlock *block, u8 *data, u32 *end,
  ...
  *end += sizeof(u32);
  ...
  for (u32 i = 0; i < block->len; ++i) {
    ...
    load_expr_data(block->items[i], data, end, path_offsets, arena, persistent_arena);
  }
}

static void load_path_offsets_data(FilePathOffsets *path_offsets, u8 *data,
                                   u32 *end, Arena *persistent_arena) {
  path_offsets->len = *(u32 *) (data + *end);
  path_offsets->cap = path_offsets->len;
  *end += sizeof(u32);

  path_offsets->items = arena_alloc(persistent_arena, path_offsets->cap * sizeof(Str));
  for (u32 i = 0; i < path_offsets->len; ++i) {
    path_offsets->items[i].offset = *end;
    load_str_data(&path_offsets->items[i].path, data, end, persistent_arena);
  }
}
...
Ir deserialize(u8 *data, u32 size, Arena *arena, Arena *persistent_arena) {
  ...
  load_path_offsets_data(&path_offsets, data, &end, persistent_arena);

  load_block_data(&ir, data, &end, &path_offsets, arena, persistent_arena);
...
Macros deserialize_macros(u8 *data, u32 size, Arena *arena, Arena *persistent_arena) {
  ...
  for (u32 i = 0; i < macros.len; ++i) {
    ...
    load_block_data(&macro->body, data, &end, &path_offsets, arena, persistent_arena);

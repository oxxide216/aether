...
Value *winx_event_to_value(WinxEvent *event, Vm *vm) {
  ...
  case WinxEventKindKeyPress: {
    ...
    dict_push_value_str_key(vm_get_frame(vm), vm->current_frame_index, &result, STR_LIT("key-code"),
                            value_string(key_code_names[key_code], vm_get_frame(vm), vm->current_frame_index));

    Str _char = { arena_alloc(&vm_get_frame(vm)->arena, 1), 1 };
    *_char.ptr = (char) event->as.key._char;
    dict_push_value_str_key(vm_get_frame(vm), vm->current_frame_index, &result, STR_LIT("char"),
                            value_string(_char, vm_get_frame(vm), vm->current_frame_index));
  } break;
  ...
  case WinxEventKindButtonPress: {
    ...
    dict_push_value_str_key(vm_get_frame(vm), vm->current_frame_index, &result, STR_LIT("button"),
                            value_string(mouse_button_names[button], vm_get_frame(vm), vm->current_frame_index));
  } break;
  ...
  case WinxEventKindMouseMove: {
    ...
    dict_push_value_str_key(vm_get_frame(vm), vm->current_frame_index, &result, STR_LIT("x"),
                            value_int(event->as.mouse_move.x, vm_get_frame(vm), vm->current_frame_index));

    dict_push_value_str_key(vm_get_frame(vm), vm->current_frame_index, &result, STR_LIT("y"),
                            value_int(event->as.mouse_move.y, vm_get_frame(vm), vm->current_frame_index));
  } break;
  ...
  case WinxEventKindResize: {
    ...
    dict_push_value_str_key(vm_get_frame(vm), vm->current_frame_index, &result, STR_LIT("width"),
                            value_float((f32) event->as.resize.width, vm_get_frame(vm), vm->current_frame_index));

    dict_push_value_str_key(vm_get_frame(vm), vm->current_frame_index, &result, STR_LIT("height"),
                            value_float((f32) event->as.resize.height, vm_get_frame(vm), vm->current_frame_index));
  } break;
  ...
  dict_push_value_str_key(vm_get_frame(vm), vm->current_frame_index, &result, STR_LIT("type"),
                          value_string(type, vm_get_frame(vm), vm->current_frame_index));
